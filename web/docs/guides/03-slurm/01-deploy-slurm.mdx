...
We might develop a HA setup in the future version of Cluster Factory.
...
Restart=always
RestartSec=3
ExecStart=/usr/sbin/slurmd -d /usr/sbin/slurmstepd --conf-server slurmctl.example.com
ExecReload=/bin/kill -HUP $MAINPID
PIDFile=/var/run/slurmd.pid
KillMode=process
LimitNOFILE=51200
...
- Mount the home diretory of the LDAP users (probably like `/home/ldap-users`)
...
The login nodes is exposed to the external network using Multus CNI and the IPVLAN plugin. This is to expose the `srunPortRange` and because the login nodes.
The users can login to the nodes using SSH via LDAP.
...
If you have configured your LDAP server, you might have change the `homeDirectory` to something like `/home/ldap-users`.

You must mount the home directory of the LDAP users using NFS.

DO NOT use `StorageClass` since the provisioning is static. We don't want to create a volume per replica. There is only one and common volume.
