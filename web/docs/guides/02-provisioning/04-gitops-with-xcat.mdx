...
A postbootscript/cloud-init file is executed after the boot of the OS, thanks to SystemD.
...
5. This file will copy files and executes other postbootscripts.
...
Here is an example of our script that we use daily at SquareFactory:
...
The copied files are stored in side a `files/` directory and other scripts are stored inside a `postscripts/` directory.
...
### Step 4: Creating the postbootscript/cloud-init file
...
On xCAT, you should add the postbootscript inside an `osimage` stanza:
